...
//			mav.setViewName("/error/404");
	...
	public void offsetDetailAjax(@PathVariable("group") String group,
		...
		String ip = request.getHeader("x-forwarded-for");
		LOG.info(new StringBuilder().append("consumer IP:").append((ip == null) ? request.getRemoteAddr() : ip).toString());
	...
	public void offsetGraphAjax(@PathVariable("group") String group,
		...
		try {
			String offsetsGraph = OffsetService.getOffsetsGraph(group, topic);

			byte[] output = GzipUtils.compressToByte(offsetsGraph);
			System.out.println(output+"==============output===="+offsetsGraph);
